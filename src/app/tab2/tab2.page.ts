...
allOrders:Order[];
  ...
  constructor(private reportService:ReportService ,private authService:AuthService, private popoverController:PopoverController, private router:Router,private orderService:OrdersService,private loadingController:LoadingController) {
    
  }
  async ngOnInit(){
    const lc  = await this.loadingController.create({message:'Please wait..',spinner:'dots'});
    await lc.present();
